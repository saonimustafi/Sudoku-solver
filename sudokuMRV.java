...
class sudokuForwardCheck
{
	...
	static boolean sudokuSolve(int[][] grid, boolean[][][] domain, PrintWriter output)
	{
		...
		if(!findUnassignedModified(grid,domain,row,col)) //This is the function with reference variables in C++ geeks implementation. SO THIS REQUIRES OBJECT REFERENCE VARIABLES TO BE PASSED THROUGH
			return true;	
	...
	static boolean findUnassignedModified(int[][] grid, boolean[][][] domain, wrapperInt row, wrapperInt col)
	{
		int rowValue;;
		int colValue;
		int minR=0,minC=0,min=100,d,flag=0; //min should be >9  (=java.lang.Integer.MAX_VALUE;)
		
		for(rowValue=0;rowValue<9;rowValue++)
		{
			for(colValue=0;colValue<9;colValue++)
			{
				if(grid[rowValue][colValue]==0) //At this point row.value and col.value attains the values of row and col which is unassigned
				{					
					d=domainSize(domain,rowValue,colValue);
					if(d<min)
					{
						min=domainSize(domain,rowValue,colValue);
						minR=rowValue;
						minC=colValue;
						flag=1;
					}										
	...
	static int domainSize(boolean[][][] domain, int rowValue, int colValue)
	{
		int i,count=0;
		for(i=0;i<9;i++)
		{
			if(domain[rowValue][colValue][i]==true)
			{
				count++;
			}
		}
